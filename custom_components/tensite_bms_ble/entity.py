...
class TensiteEntity(PassiveBluetoothCoordinatorEntity[TensiteClusterCoordinator]):
    ...
    check either. Home Assistant suppresses repeat advertisements whose content
    has not changed, and this gateway broadcasts constant manufacturer data, so
    its advertisements only reach us every few minutes and Home Assistant marks
    it unavailable in between. Tying entities to that made every reading flap
    for most of each interval despite polls succeeding perfectly.
